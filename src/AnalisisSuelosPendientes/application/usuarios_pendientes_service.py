...
class UsuariosPendientesService:
    ...
    def get_usuarios_con_pendientes(db: Session) -> Dict[str, Any]:
        """
        Obtiene todos los usuarios que tienen análisis de suelos pendientes con estadísticas completas
        """
        try:
            print("Obteniendo usuarios con análisis pendientes...")
            
            # Query para obtener usuarios con análisis pendientes
            usuarios_query = (
                db.query(
            ...
            for usuario in usuarios_query:
                # Obtener municipios únicos para este usuario
                municipios_query = (
                ...
                print(f"Usuario {usuario.correo_usuario}: {usuario.total_pendientes} pendientes en {len(municipios_involucrados)} municipios")
